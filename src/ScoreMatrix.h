...
struct ScoreMatrix {
    int alphabetLength;
    char* alphabet; //!< letters in same order as columns/rows in matrix
    int* matrix; //!< has dimensions of [alphabetLength, alphabetLength]
};

//--------------------------------------------- BLOSUM -----------------------------------------------//
char blosumAlphabet[24] = {'A', 'R', 'N', 'D', 'C', 'Q', 'E', 'G', 'H', 'I', 'L', 'K', 
                           'M', 'F', 'P', 'S', 'T', 'W', 'Y', 'V', 'B', 'Z', 'X', '*'}; 

int blosum50Matrix[24*24] = {
...
ScoreMatrix getBlosum50() {
    ...
    scoreMatrix.alphabetLength = 24;
    scoreMatrix.alphabet = blosumAlphabet;
    scoreMatrix.matrix = blosum50Matrix;
    return scoreMatrix;
}
//----------------------------------------------------------------------------------------------------//
